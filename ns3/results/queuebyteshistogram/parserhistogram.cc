   ...
   pugi::xpath_node_set histogram = doc.select_nodes("/FlowMonitor/FlowStats/Flow");
	...
	for (pugi::xpath_node_set::const_iterator it = histogram.begin(); it != histogram.end(); ++it)
	{
	   ...
	   bool found = (std::find(ovhFlows.begin(), ovhFlows.end(), flowId) != ovhFlows.end());
		...
		if (found)
		   continue;
	   for (pugi::xml_node histogram = node.node().child("avgQueueSizeHistogram"); histogram; histogram = histogram.next_sibling("avgQueueSizeHistogram")) {
...
int main(int argc,char *argv[])
{
   ...
   long *binarray = (long*)malloc(sizeof(long) * 5 * bcount);//new long [5][bcount];
   ...
   for (int i = 0; i < bcount/compressionfactor; i++) {
      ...
      queuesizes << " " << i*100*compressionfactor;
   }

	ostringstream delays;
   ostringstream ticks;
   for (int i = 1; i <= bcount; i = i + 2) {
      delays << ", " <<  1 * i; 
      ticks << ", '" <<  100 * compressionfactor * i << "'";

   }
   
   ofstream octavefile;
  	octavefile.open ("queuebytes.m");

	octavefile << "function queuebytes()" << endl;
   ...
   octavefile << "set (h(5), 'facecolor', 'yellow');" << endl;
	...
	octavefile << "xlabel('Observed output queue size [B]');" << endl;
	...
	octavefile << "print -djpg queuebyteshistogram.jpg" << endl;
