...
//
// Edited by Natasa Maksic <maksicn@etf.rs> (Added support for flow-based ECMP)
//
...
Ipv4GlobalRouting::GetTupleValue (const Ipv4Header &header, Ptr<const Packet> ipPayload)
{
   ...
   uint32_t tupleValue = header.GetSource ().Get () + header.GetDestination ().Get () + header.GetProtocol ();
   uint32_t src = header.GetSource ().Get ();
   uint32_t dst = header.GetDestination ().Get ();

   for (int i=0; i<=3; i++) {
      buffer[i] = (src >> 8*(3-i)) & 0xff;
      buffer[i+4] = (dst >> 8*(3-i)) & 0xff;  
   } 
   buffer[8] = header.GetProtocol ();
   uint16_t srcport = 0;
   uint16_t dstport = 0;
   ...
   for (int i=0; i<=1; i++) {
      buffer[i+9] = (srcport >> 8*(1-i)) & 0xff;
      buffer[i+11] = (dstport >> 8*(1-i)) & 0xff;
   }
