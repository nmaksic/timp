...
//
// Edited by Natasa Maksic <maksicn@etf.rs> (Added support for queue size histogram
// and for counting transmitted bytes on all ports along the path)
//
...
FlowMonitor::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::FlowMonitor")
    ...
    .AddAttribute ("FlowInterruptionsMinTime", ("The minimum inter-arrival time that is considered a flow interruption."),
                   ...
                   MakeTimeChecker ())
    .AddAttribute ("AverageQueueSizeBinWidth", ("The width used in the average queue size histogram."),
                   DoubleValue (10),
                   MakeDoubleAccessor (&FlowMonitor::m_avgQueueSizeBinWidth),
                   MakeDoubleChecker <double> ())
  ;
  return tid;
}
  ...
  if (iter == m_flowStats.end ())
    {
      ...
      ref.avgQueueSizeHistogram.SetDefaultBinWidth (m_avgQueueSizeBinWidth);
      ref.txBytesEachPort = 0;
      return ref;
    }
  else
    {
...
void
FlowMonitor::ReportFirstTx (Ptr<FlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize, uint32_t queueSize)
{
  ...
  tracked.queueSizeSum = queueSize;
  ...
  stats.txBytesEachPort += packetSize;
  stats.txPackets++;
...
void
FlowMonitor::ReportForwarding (Ptr<FlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize, uint32_t queueSize)
{
  NS_LOG_FUNCTION (this << probe << flowId << packetId << packetSize);
  if (!m_enabled)
    {
  ...
  tracked->second.queueSizeSum += queueSize;
...
FlowMonitor::ReportLastRx (Ptr<FlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize)
{
  ...
  stats.avgQueueSizeHistogram.AddValue (tracked->second.queueSizeSum/(tracked->second.timesForwarded + 1));
      ...
      if (enableHistograms)
        {
          ...
          flowI->second.avgQueueSizeHistogram.SerializeToXmlStream (os, indent, "avgQueueSizeHistogram");
        }
      indent -= 2;
