...
Ipv6FlowProbe::SendOutgoingLogger (const Ipv6Header &ipHeader, Ptr<const Packet> ipPayload, uint32_t interface)
{
  ...
  if (m_classifier->Classify (ipHeader, ipPayload, &flowId, &packetId))
    {
      Ptr< NetDevice > device = m_ipv6->GetNetDevice (interface);
      PointerValue queue0Value;
      device->GetAttribute ("TxQueue", queue0Value);
		Ptr<Queue<Packet>> queue0 = queue0Value.Get<Queue<Packet>> ();
		uint32_t queuepackts = queue0->GetNPackets();
    
      uint32_t size = (ipPayload->GetSize () + ipHeader.GetSerializedSize ());
      NS_LOG_DEBUG ("ReportFirstTx ("<<this<<", "<<flowId<<", "<<packetId<<", "<<size<<"); "
                                     << ipHeader << *ipPayload);
      m_flowMonitor->ReportFirstTx (this, flowId, packetId, size, queuepackts);
  ...
  if (found)
    {
      ...
      m_flowMonitor->ReportForwarding (this, flowId, packetId, size, queuepackts);
    }
}
