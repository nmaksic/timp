...
public:
  ...
  struct FlowStats
  {
    ...
    /// Total number of transmitted bytes belonging to flow on each port 
    uint64_t txBytesEachPort;
    ...
    /// Histogram of the average queue size along packet path
    Histogram avgQueueSizeHistogram;
  ...
  /// \param queueSize queue size on output port
  void ReportFirstTx (Ptr<FlowProbe> probe, FlowId flowId, FlowPacketId packetId, uint32_t packetSize, uint32_t queueSize);
  /// FlowProbe implementations are supposed to call this method to
  ...
  /// \param packetSize packet size
  /// \param queueSize queue size on output port
  void ReportForwarding (Ptr<FlowProbe> probe, FlowId flowId, FlowPacketId packetId, uint32_t packetSize, uint32_t queueSize);
  /// FlowProbe implementations are supposed to call this method to
...
private:
  ...
  double m_avgQueueSizeBinWidth;   //!< Average queue size bin width (for histograms)
